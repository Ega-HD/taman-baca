    ...
    public function run()
    {
        ...
        for ($i = 0; $i < 30; $i++) {
            $members[] = User::create([
                ...
                'password'     => Hash::make('password'),
                'role'         => 'member',
                // [FIX] Format No HP selalu angka 12 digit tanpa spasi/simbol
        ...
        echo "✅ 30 Member Dummy & 1 Admin dibuat (Format HP 08... numeric).\n";

        // [FIX] DAFTAR JUDUL BUKU REALISTIS UNTUK PAUD
        $judul_buku_real = [
            'Belajar Membaca Tanpa Mengeja', 'Kisah 25 Nabi dan Rasul', 'Kancil Anak Nakal',
            'Dongeng Nusantara Pilihan', 'Buku Pintar Anak Islam', 'Mengenal Hewan dan Tumbuhan',
            'Bawang Merah dan Bawang Putih', 'Panduan Mendidik Anak Usia Dini', 'Ensiklopedia Anak Pintar',
            'Mewarnai Pemandangan Alam', 'Kisah Sahabat Nabi', 'Buku Latihan Menulis Huruf Hijaiyah',
            'Cerita Rakyat Nusantara', 'Seri Sains Anak: Tata Surya', 'Mengenal Angka dan Berhitung',
            'Kamus Bergambar 3 Bahasa', 'Dongeng Sebelum Tidur', 'Seri Karakter Anak Hebat',
            'Panduan Gizi untuk Balita', 'Buku Aktivitas PAUD Cerdas', 'Pangeran Berkelana Tanpa Pelana',
            'Katak Terjungkir Bulan Tiada', 'Pengusaha Sukses Sejak PAUD', 'Tikus Bumi Satu Satunya'
        ];
        ...
        for ($i = 0; $i < 24; $i++) {
            $katalog_buku[] = Buku::create([
                // Mengambil judul unik dari array di atas
                'judul_buku'   => $judul_buku_real[$i], 
                'penulis'      => $faker->name,
                'penerbit'     => $faker->company,
                'tahun_terbit' => $faker->numberBetween(2015, 2023), // Tahun lebih logis
                'created_at'   => Carbon::now(),
            ]);
        }
        echo "✅ 20 Katalog Buku Realistis dibuat.\n";

        // 5. BUAT ITEM BUKU FISIK & TRANSAKSI
        echo "🔄 Membuat Fisik Buku & Menyinkronkan Transaksi...\n";

        foreach ($katalog_buku as $buku) {
            $jumlah_copy = rand(4, 8);
            ...
            for ($urutan = 1; $urutan <= $jumlah_copy; $urutan++) {
                ...
                // Penentuan Nasib Buku (Random)
                $rand = rand(1, 100);
                $peminjam = $faker->randomElement($members);
                
                // Variabel yang akan disinkronkan
                $status_buku = 'Tersedia';
                $status_transaksi = null;

                // --- LOGIKA SINKRONISASI MUTLAK ---
                if ($rand <= 20) { 
                    $status_buku = 'Dipinjam';
                    $status_transaksi = 'Sedang Dipinjam';
                } elseif ($rand <= 30) { 
                ...
                // 1. SIMPAN ITEM BUKU (Statusnya sudah pasti sesuai dengan transaksi nanti)
                $itemBuku = ItemBuku::create([
                    'buku_id'         => $buku->id,
                    'kode_buku'       => $kode_buku,
                    'status_buku'     => $status_buku,
                ...
                // 2. SIMPAN TRANSAKSI BERSASARKAN STATUS BUKU
                
                // [KASUS A] Buku sedang keluar rak (Sedang Dipinjam / Menunggu Pengembalian / Di-booking)
                if ($status_buku === 'Dipinjam' || $status_buku === 'Di-booking') {
                    ...
                    if ($status_transaksi === 'Menunggu Persetujuan') {
                        $dataTransaksi['tgl_pengajuan_pinjam'] = Carbon::now()->subMinutes(rand(10, 300));
                        $dataTransaksi['tarif_denda_berlaku'] = null; // Belum di-ACC
                    } else {
                        // Jika Sedang Dipinjam atau Menunggu Pengembalian (Sudah di-ACC)
                        $dataTransaksi['approved_by_id'] = $admin->id;
                        $dataTransaksi['tgl_pengajuan_pinjam'] = $tgl_pinjam->copy()->subHours(2);
                        $dataTransaksi['tgl_disetujui'] = $tgl_pinjam;
                        $dataTransaksi['tgl_pinjam'] = $tgl_pinjam;
                        $dataTransaksi['deadline'] = $tgl_pinjam->copy()->addDays(7);
                        $dataTransaksi['tarif_denda_berlaku'] = $tarif_denda_default;
                    }
                ...
                // [KASUS B] Buku Tersedia (Ada kemungkinan punya History masa lalu)
                elseif ($status_buku === 'Tersedia') {
                    // 50% peluang buku ini pernah dipinjam sebelumnya
                    if (rand(1, 100) <= 50) {
